...
class exportadmissionlist implements FromView, ShouldAutoSize, WithEvents, WithDrawings
{
    ...
    public function __construct($start, $end , $doctor)
    {
        ...
        if($this->doctor =="all"){
            ...
            $this->admissionlistreport = DB::table('v_report_admission_list')
            ->where('START_DATE', '>=', $start)
            ->where('START_DATE', '<=', $end)
            ->get();
            $this->admissionlistreportcount = DB::table('v_report_admission_list')
            ->where('START_DATE', '>=', $start)
            ->where('START_DATE', '<=', $end)
            ->count();
        }
    ...
    public function view(): View
    {
        ...
        return view('livewire.report.admissionlist-excel',["imageLogo"=>$this->imageLogo,"reportcount"=>$this->admissionlistreportcount
        ,"admissionlistreport"=>$this->admissionlistreport,"start"=>$this->start,"end"=>$this->end]);
    }
}
